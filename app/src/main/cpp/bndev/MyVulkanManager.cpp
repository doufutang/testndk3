...
void MyVulkanManager::create_vulkan_swapChain()
{
    ...
    PFN_vkCreateAndroidSurfaceKHR fpCreateAndroidSurfaceKHR=//动态加载创建KHR表面的方法
            (PFN_vkCreateAndroidSurfaceKHR)vk::vkGetInstanceProcAddr(instance,"vkCreateAndroidSurfaceKHR");//加载Android平台所需方法
    ...
    result = vk::vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[0],surface, &formatCount, NULL);//获取支持的格式数量
    ...
    result = vk::vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[0], surface, &formatCount, surfFormats);//获取支持的格式信息
    ...
    for (size_t i = 0; i < presentModeCount; i++)//遍历显示模式列表
    {
        ...
        if ((swapchainPresentMode != VK_PRESENT_MODE_MAILBOX_KHR)&&(presentModes[i] == VK_PRESENT_MODE_IMMEDIATE_KHR))//若支持IMMEDIATE模式
        {
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)//为交换链中的各幅图像创建图像视图
    {
        ...
        result = vk::vkCreateImageView(device,&color_image_view, NULL, &swapchainImageViews[i]);//创建图像视图
        assert(result == VK_SUCCESS);//检查是否创建成功
    }
}
...
void MyVulkanManager::create_render_pass()
{
    ...
    VkResult result = vk::vkCreateSemaphore(device,&imageAcquiredSemaphoreCreateInfo, NULL, &imageAcquiredSemaphore);//创建信号量
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){//每循环一次绘制一帧画面
        ...
        VkResult result = vk::vkAcquireNextImageKHR(device, swapChain,UINT64_MAX, imageAcquiredSemaphore, VK_NULL_HANDLE,&currentBuffer);//获取交换链中的当前帧索引
