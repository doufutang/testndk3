...
void ShaderQueueSuit_Common::create_shader(VkDevice& device)
{
    ...
    VkResult result = vk::vkCreateShaderModule(device,&moduleCreateInfo, NULL, &shaderStages[0].module);//创建顶点着色器模块
    ...
    result = vk::vkCreateShaderModule(device,&moduleCreateInfo, NULL, &shaderStages[1].module);//创建片元着色器模块
...
void ShaderQueueSuit_Common::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
    ...
    VkResult result = vk::vkCreatePipelineCache(device, &pipelineCacheInfo, NULL,&pipelineCache);//创建管线缓冲
    assert(result == VK_SUCCESS);//检查管线缓冲创建是否成功
    result = vk::vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineInfo, NULL, &pipeline);//创建管线
